    ...
    overlay.innerHTML =
      '<div class="dk-card v-'+variant+(maint?' dk-maint':'')+'" role="dialog" aria-modal="true">'+
        '<div class="dk-head">'+
          ...
          '<div class="dk-htext">'+
            '<div class="dk-brand">RabbitHub <em>•</em> ANIMES</div>'+
  ...
  function boot(){
    ...
    try {
      db.ref('dk_broadcast/config/forcedTheme').on('value', function(s){
        var v = s.val();
        if (!v || !window.DK_setTheme) return;
        /* Em páginas com plano, Free permanece azul e VIP conserva a
           escolha individual. O tema global é usado somente nas telas
           públicas que não carregam RabbitHubPlans (ex.: login). */
        if (window.RabbitHubPlans && window.RabbitHubPlans.ready) {
          window.RabbitHubPlans.ready().then(function(access){
            if (access && access.customTheme === true) {
              if (window.DK_verifyTheme) window.DK_verifyTheme();
            } else {
        ...
        window.DK_setTheme(v,false,{system:true});
      });
    } catch(e){}
    ...
    db.ref('dk_broadcast/maintenance').on('value', function(snap){
      ...
      if (m && m.enabled){
        ...
        maintOpen = showModal({
          ...
          body:  m.body  || 'O RabbitHub está passando por uma atualização rápida. Volte em instantes!',
          until: m.until || '',
          variant:'warning',
          dismissible:false,
          maintenance:true
